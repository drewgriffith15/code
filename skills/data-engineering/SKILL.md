...
Direct entry: use this skill when you already know the work is Oracle 19c data engineering.
...
Start by shaping the work before generating artifacts or proposing rewrites.

- Read the supplied SQL or PL/SQL and identify the target procedure, package, or anonymous block.
- Identify the real objective: diagnostics only, test harness, rewrite, comparison, deployment prep, or closeout.
- Extract every referenced object that matters to the database footprint, including tables, views, packages, sequences, log tables, and helper routines.
- State the current understanding of the object graph, likely risk areas, validation path, and any assumptions that still need confirmation.
- Record the session start timestamp so Phase 5 can report actual duration.
- Write or update the active project PRD in `prd/` before deeper implementation work begins.
- If the task is ambiguous, resolve the ambiguity in the PRD and in chat before generating downstream artifacts.

## Workflow

1. Produce a diagnostics-first assessment before any rewrite work begins.
2. Generate a single consolidated diagnostic SQL artifact in `sandbox/` that can be pasted into a worksheet and run in one pass.
3. Pause after diagnostics and wait for human review before moving to test-table, rewrite, or comparison steps.
4. After approval, use the diagnostics output to design the test-table or test-harness script in `sandbox/`.
5. After the test table is validated, generate the proposed rewrite in `sandbox/`, and default the first rewrite pass to the `_TEST` target table for human validation.
6. After the test rewrite is run, generate comparison SQL that checks production versus test outputs.
7. After the comparison review is approved, generate a deployment-ready splice or procedure draft that removes `_TEST` from the target object references.
8. Pause again for human approval before applying the deployment-ready procedure into the package or source file.
9. After approval and commit, write a concise work note in `worknotes/`.
10. Generate an explicit cleanup SQL artifact when the workflow created temporary tables or other throwaway validation objects.
11. Do not clear `sandbox/` until the user confirms the work is closed out and any needed commit has already been made.
12. When outputting any artifact, show the exact absolute file path where it was written.
13. Keep generated SQL worksheet-safe by default: use `--` comment headers, not SQL*Plus `SET` or `PROMPT` commands.
...
Run a final review after the workflow has been validated and before recommending closure.

### Axis 1 - Modularity

- Check touched SQL, PL/SQL, and helper artifacts for oversized blocks, duplicated logic, weak naming, or preventable complexity.
- Tighten only the files touched in this build. Do not expand into unrelated refactors.

### Axis 2 - Security

- Check for hardcoded credentials, unsafe environment-specific paths, or logs and comments that expose PII or student data.
- Fix issues found in the touched artifacts and keep outputs compliant with FERPA-sensitive environments.

### Axis 3 - Doc Sync

- Update the active PRD in `prd/` so it matches the final approved implementation path.
- Update any project `CLAUDE.md` or `README.md` files when the work changed conventions, commands, or file layout.
...
- After saving the work note file, paste the same ServiceNow-ready note block directly into chat.
